...
module m3_halfbolt_hole ()
{
  union () {
    ...
    translate([0,0,-1])
      ...
      cylinder (r= m3_head_r_tol, h= m3_head_l+1+1, $fa=1, $fs=0.5);  
  ...
  // they are outside the hexagon
    intersection () {  
      rotate([0,0,30])translate([0,0,m3_head_l+1])
        cylinder (r= m3_r_tol*2, h=h_layer3d, $fn=3);
      // take vetexs away:
      translate([0,0,m3_head_l+1])
        cylinder (r= m3_head_r_tol, h= h_layer3d, $fa=1, $fs=0.5);    
    }
    
    // 1.15 is the relationship between the Radius and the Apothem
    // of the hexagon: sqrt(3)/2 . I make it slightly smaller
    translate([0,0,m3_head_l+1+h_layer3d])
      //rotate ([0,0,30])
...
module m4_halfbolt_hole ()
{
  union () {
    ...
    translate([0,0,-1])
      ...
      cylinder (r= m4_head_r_tol, h= m4_head_l+1+1, $fa=1, $fs=0.5);  
  ...
  // they are outside the hexagon
    intersection () {  
      rotate([0,0,30])translate([0,0,m4_head_l+1])
        cylinder (r= m4_r_tol*2, h=h_layer3d, $fn=3);
      // take vetexs away:
      translate([0,0,m4_head_l+1])
        cylinder (r= m4_head_r_tol, h= h_layer3d, $fa=1, $fs=0.5);    
    }
    
    // 1.15 is the relationship between the Radius and the Apothem
    // of the hexagon: sqrt(3)/2 . I make it slightly smaller
    translate([0,0,m4_head_l+1+h_layer3d])
      //rotate ([0,0,30])
